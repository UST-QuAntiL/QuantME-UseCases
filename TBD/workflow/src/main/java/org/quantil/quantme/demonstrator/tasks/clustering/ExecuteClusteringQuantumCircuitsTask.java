...
import org.quantil.quantme.demonstrator.requests.clustering.ExecuteQuantumCircuitsRequest;
...
public class ExecuteClusteringQuantumCircuitsTask implements JavaDelegate {

    private final static Logger LOGGER = LoggerFactory.getLogger(GenerateClusteringQuantumCircuitsTask.class);

    @Override
    public void execute(DelegateExecution execution) throws Exception {
        LOGGER.info("Sending request to execute circuits for the k-means iteration...");
        ...
        // create request
        final ExecuteQuantumCircuitsRequest request = new ExecuteQuantumCircuitsRequest();
        request.setCircuitsUrl(
                Utils.getUrlToProcessVariable(execution.getProcessInstanceId(), Constants.VARIABLE_NAME_CIRCUITS_FILE));
        ...
        request.setK(2);
        LOGGER.info("Circuits URL: {}", request.getCircuitsUrl());
        ...
        try {
            ...
            // move centroids of the performed iteration to the old centroids variable
            final FileValue retrievedTypedFileValue = execution.getProcessEngineServices().getRuntimeService()
                    .getVariableTyped(execution.getId(), Constants.VARIABLE_NAME_CENTROID_FILE);
            execution.setVariable(Constants.VARIABLE_NAME_OLD_CENTROID_FILE, retrievedTypedFileValue);
    ...
    private String getRequestUrl(int jobId) {
        return Configuration.getInstance().properties.getProperty("clustering")
                + Constants.CLUSTERING_API_CIRCUIT_EXECUTION + jobId;
    }
}
