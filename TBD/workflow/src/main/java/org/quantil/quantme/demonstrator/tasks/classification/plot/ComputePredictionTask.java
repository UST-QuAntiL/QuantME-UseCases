...
import org.quantil.quantme.demonstrator.requests.classification.plot.ComputePredictionRequest;
...
public class ComputePredictionTask implements JavaDelegate {

    private final static Logger LOGGER = LoggerFactory.getLogger(ComputePredictionTask.class);

    @Override
    public void execute(DelegateExecution execution) throws Exception {
        LOGGER.info("Sending request to compute prediction...");
        ...
        // create request
        final ComputePredictionRequest request = new ComputePredictionRequest();
        request.setResultUrl(Utils.getUrlToProcessVariable(execution.getProcessInstanceId(),
        ...
        try {
            ...
            final URL predictionsUrl = new URL(jo.get(Constants.CLASSIFICATION_RESPONSE_PREDICTIONS_URL).toString());

            // download results and add to variables
            final boolean success = Utils.addFileFromUrlAsVariable(predictionsUrl, "predictions-", null,
                    Constants.VARIABLE_NAME_CLASSIFICATION_PREDICTIONS_FILE, MediaType.TEXT_PLAIN, execution);
    ...
    private String getRequestUrl(int jobId) {
        return Configuration.getInstance().properties.getProperty("classification")
                + Constants.CLASSIFICATION_API_PREDICTION + jobId;
    }
}
