...
import org.quantil.quantme.demonstrator.requests.classification.plot.GenerateGridRequest;
...
public class GenerateGridTask implements JavaDelegate {

    private final static Logger LOGGER = LoggerFactory.getLogger(GenerateGridTask.class);

    @Override
    public void execute(DelegateExecution execution) throws Exception {
        LOGGER.info("Generating grid for decision boundary...");
        ...
        // create request
        final GenerateGridRequest request = new GenerateGridRequest();
    ...
    private String getRequestUrl(int jobId) {
        return Configuration.getInstance().properties.getProperty("classification") + Constants.CLASSIFICATION_API_GRID
                + jobId;
    }
}
