...
import org.quantil.quantme.demonstrator.requests.clustering.InitializeCentroidsRequest;
...
public class InitializeCentroidsTask implements JavaDelegate {

    private final static Logger LOGGER = LoggerFactory.getLogger(InitializeCentroidsTask.class);

    @Override
    public void execute(DelegateExecution execution) throws Exception {
        LOGGER.info("Sending request to initialize centroids for k-means...");

        // generate job id for the clustering which is required by the clustering
        // microservice
        final int jobId = new Random().nextInt(1000);
        execution.setVariable(Constants.VARIABLE_NAME_CLUSTERING_JOB_ID, jobId);

        // variable to track number of clustering iterations
        execution.setVariable(Constants.VARIABLE_NAME_CLUSTERING_ITERATIONS, 0);

        // create request
        final InitializeCentroidsRequest request = new InitializeCentroidsRequest();
        request.setK(2);
    ...
    private String getRequestUrl(int jobId) {
        return Configuration.getInstance().properties.getProperty("clustering")
                + Constants.CLUSTERING_API_INITIALIZE_CENTROIDS + jobId;
    }
}
