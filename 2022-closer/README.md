...
Then, the following modal is displayed, comprsing some information about the analysis and rewrite method:

![Hybrid Loop Detection Modal](./docs/hybrid-loop-detection-modal.png)

Press the ``Analyse Workflow`` button to start the detection of hybrid loop within the workflow, which could benefit from a hybrid runtime.
...
In our example workflow, two optimization candidates are detected, one performing the clustering, and the second comprsing the classification tasks.
...
Click on the ``Rewrite Workflow`` button to analyse if the selected hybrid runtime supports the current candidate.
If yes, a hybrid program, as well as a corresponding deployment model, are automatically generated for the candidate.
Finally, the workflow is rewriten to invoke the generated hybrid program.
